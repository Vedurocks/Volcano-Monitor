...
VOLCANO_API  = "https://volcano-monitor-g4hl.vercel.app/api/volcano"
PRISMA_DB    = "postgres://4d25951a202fb0d6375a443e4d190433c772440e3eb8d832c99a06f3824dfa76:sk_qUug2x2t-TL6c5GRBBt47@db.prisma.io:5432/postgres?sslmode=require"
CLOUD_SEND_INTERVAL = 2.5  # seconds between cloud updates (2-3 seconds as per spec)
GRAPH_POINTS = 120
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Cloud sync state
        self._last_cloud_send = 0  # timestamp of last cloud update
        self._cloud_cache = []     # cache for failed requests
        self._last_data = None     # buffer latest data for throttled sending
    ...
    def _send_to_cloud(self, dist, seis, dist_alert, seis_alert, 
                       dual_alert, kill, sys_on, min_ultra, min_seis):
        """Buffer data and send to cloud every 2-3 seconds (throttled, non-blocking)"""
        if not HAS_REQUESTS:
            return
        
        # Buffer latest data (will be sent on next throttle interval)
        self._last_data = {
            'dist': dist,
            'seis': seis,
            'dist_alert': dist_alert,
            'seis_alert': seis_alert,
            'dual_alert': dual_alert,
            'kill': kill,
            'sys_on': sys_on,
            'min_ultra': min_ultra,
            'min_seis': min_seis
        ...
        # Check if enough time has passed (throttle to 2-3 seconds)
        now = time.time()
        if now - self._last_cloud_send < CLOUD_SEND_INTERVAL:
            return  # too soon, skip this cycle
        
        self._last_cloud_send = now
        
        # Send in background thread
        threading.Thread(target=self._cloud_worker, daemon=True).start()
    
    def _cloud_worker(self):
        """Worker thread: send buffered data + retry cached failed requests"""
        if not self._last_data:
            return
        
        data_dict = self._last_data
        
        # Format: distance,vibration_count,dist_alert,vib_alert,dual_alert,kill,sys,min_ultra,min_seis
        payload = (
            f"{data_dict['dist']:.2f},"
            f"{int(data_dict['seis'])},"
            f"{'DIST_ALERT' if data_dict['dist_alert'] else 'DIST_OK'},"
            f"{'VIB_ALERT' if data_dict['seis_alert'] else 'VIB_OK'},"
            f"{'DUAL_ALERT' if data_dict['dual_alert'] else 'DUAL_OK'},"
            f"{'KILL_ON' if data_dict['kill'] else 'KILL_OFF'},"
            f"{'SYS_ON' if data_dict['sys_on'] else 'SYS_OFF'},"
            f"{data_dict['min_ultra']:.1f},"
            f"{int(data_dict['min_seis'])}"
        )
        
        # Try to send current data
        success = self._post_to_api(payload)
        
        if not success:
            # Cache failed request (keep last 100 max)
            self._cloud_cache.append({
                'timestamp': time.time(),
                'payload': payload
            })
            if len(self._cloud_cache) > 100:
                self._cloud_cache.pop(0)  # remove oldest
        
        # Retry cached failed requests (max 5 per cycle to avoid overload)
        retry_count = 0
        while self._cloud_cache and retry_count < 5:
            cached = self._cloud_cache[0]
            
            # Don't retry items older than 5 minutes
            if time.time() - cached['timestamp'] > 300:
                self._cloud_cache.pop(0)
                continue
            
            if self._post_to_api(cached['payload']):
                self._cloud_cache.pop(0)  # success, remove from cache
                retry_count += 1
            else:
                break  # failed, stop retrying for now
    
    def _post_to_api(self, payload):
        """Send single POST request to Vercel API. Returns True on success."""
        try:
            ...
            if 200 <= response.status_code < 300:
                ...
                # Log failure (optional - uncomment for debugging)
                # print(f"✗ Cloud API error: {response.status_code}")
                return False
                
        except Exception as e:
            # Network error, timeout, etc. - cache for retry
            # print(f"✗ Cloud connection error: {e}")
            return False
    
    def _send_startup_message(self):
        """Send startup message to cloud on first connection"""
        if not HAS_REQUESTS:
            return
        
        def _post():
            try:
                # Format: MIN_ULTRA:10.0,MIN_VIB:3
                data = f"MIN_ULTRA:{self._alarm_dist:.1f},MIN_VIB:{int(self._alarm_seis)}"
                ...
                # if 200 <= response.status_code < 300:
                #     print(f"✓ Startup message sent: {response.status_code}")
                
            except Exception:
                # Startup message failure is non-critical
                pass
