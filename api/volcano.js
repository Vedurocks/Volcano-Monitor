  ...
  try {
    ...
    if (req.method === 'GET') {
      const { rows: data } = await sql`
        SELECT * FROM volcano_data ORDER BY created_at DESC LIMIT 1
      `;
      const { rows: cmds } = await sql`
        SELECT id, command FROM volcano_commands
        WHERE executed = false ORDER BY created_at ASC
      `;
      return res.status(200).json({ data: data[0] || null, commands: cmds });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (body.executed_id) {
        await sql`UPDATE volcano_commands SET executed = true WHERE id = ${body.executed_id}`;
        return res.status(200).json({ ok: true });
      }

      if (body.command) {
        const { rows } = await sql`
          INSERT INTO volcano_commands (command) VALUES (${body.command}) RETURNING id
        `;
      ...
      await sql`DELETE FROM volcano_data WHERE created_at < NOW() - INTERVAL '1 hour'`;
